...
-<a href="#1042">1042. 字符统计(20)</a>  
-<a href="#1043">1043. 输出PATest(20)</a>  
-<a href="#1044">11044. 火星数字(20)</a>  
...
思路：计算最简分数形式，分别按照基本加减乘除运算求值，按要求打印，注意题目要求输出为整型，而中间计算过程可能会出现越界情况，内容比较复杂，把功能分开写有助于理清思路。  
...
在求 num_AT 的时候就取余数，可以避开数字超过整型范围的问题。  
数字求和取余数，可以分别取余数在求和。  

<a href="#catalogue"> back to catalogue </a>  


##<p id="1041">1042. 字符统计(20)</p>  
...
思路：结构体保存信息，字符串数组保存14位准考证号，两层循环查找对应考生即可。
问题：结构体中的数组读取数据，注意scanf  取地址。

<a href="#catalogue"> back to catalogue </a>  


##<p id="1042">1042. 字符统计(20)</p>  
...
思路：保存输入字符串，遍历一遍，英文字符次数保存到一个长度为26的数组中。找到数组中最大的值，输出即可。  
问题：scanf 不能输入空格，使用正则表达式，遇到 '\n' 停止输入，如下  
>
    scanf ("%[^\n]", input_string);
...
思路：一个长度 6 的数组记录PATest 各个字母出现的次数，依据这个次数打印。  
问题：打印有两层循环，外层是出现次数最多的个数，内层是 6 个字母循环。  

<a href="#catalogue"> back to catalogue </a>  


##<p id="1044">1044. 火星数字(20)</p>  
火星人是以13进制计数的：  
地球人的0被火星人称为tret。  
