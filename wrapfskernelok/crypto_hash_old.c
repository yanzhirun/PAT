...
#define MY_FILE "/home/yzr/work/11/my_passwd"


static int wrapfs_passwd_store(const char* file_name, const char* random )
{
	...
	file->f_op->write(file, (char *)file_name, strlen(file_name), &file->f_pos);
	file->f_op->write(file, (char *)random, strlen(random), &file->f_pos);
	set_fs(old_fs);
