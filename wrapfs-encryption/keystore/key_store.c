...
int wrapfs_key_store(char *file_name, char *user_name, char *passwd)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	vfs_write(file, (char *)user_name, strlen(user_name), &file->f_pos);
	vfs_write(file, INTERNAL_FLAG, strlen(INTERNAL_FLAG), &file->f_pos);
	vfs_write(file, (char *)passwd, strlen(passwd), &file->f_pos);
	vfs_write(file, BEGIN_FLAG, strlen(BEGIN_FLAG), &file->f_pos);
	set_fs(old_fs);
...
int wrapfs_get_key(char *file_name, char *user_name, char *passwd)
{
	...
	read_num = vfs_read(file, buf, sizeof(buf), &file->f_pos);
	if(read_num == 0)
	{
